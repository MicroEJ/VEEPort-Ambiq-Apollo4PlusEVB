...
#define KEEP_COORD(p,n,limit)		(((DIFF(p,n)) <= (limit)) ? (bool)MICROEJ_FALSE : (bool)MICROEJ_TRUE)
#define KEEP_PIXEL(px,x,py,y,limit)	((KEEP_COORD(px,x,limit)) || (KEEP_COORD(py,y,limit)))
#define KEEP_FIRST_MOVE(px,x,py,y)	(KEEP_PIXEL(px,x,py,y, (int32_t)FIRST_MOVE_PIXEL_LIMIT))
#define KEEP_MOVE(px,x,py,y)		(KEEP_PIXEL(px,x,py,y, (int32_t)MOVE_PIXEL_LIMIT))

/* Global --------------------------------------------------------------------*/

static uint8_t touch_pressed = MICROEJ_FALSE;
// cppcheck-suppress [misra-c2012-8.9] needs to be global
static uint8_t touch_moved = MICROEJ_FALSE;	// == MICROEJ_TRUE after the first "move" event
// cppcheck-suppress [misra-c2012-8.9] needs to be global
static uint16_t previous_touch_x;
// cppcheck-suppress [misra-c2012-8.9] needs to be global
static uint16_t previous_touch_y;
...
void TOUCH_HELPER_pressed(int32_t x, int32_t y)
{
	...
	if (touch_pressed == MICROEJ_TRUE)
	{
		...
		{
			keep_pixel = KEEP_MOVE((int32_t)previous_touch_x, x, (int32_t)previous_touch_y, y);
		}
		else
		{
			keep_pixel = KEEP_FIRST_MOVE((int32_t)previous_touch_x, x, (int32_t)previous_touch_y, y);
		}

		if (keep_pixel == (bool)MICROEJ_TRUE)
		{
			// store the new pixel
			previous_touch_x = (uint16_t)x;
			previous_touch_y = (uint16_t)y;
			touch_moved = MICROEJ_TRUE;

			// send a MicroUI touch event (don't care if event is lost)
			(void) EVENT_GENERATOR_touch_moved(x, y);
		}
	...
	else
	{
		...
		if (EVENT_GENERATOR_touch_pressed(x, y) == (int32_t)LLUI_INPUT_OK)
		{
...
void TOUCH_HELPER_released(void)
{
	...
	if (touch_pressed == MICROEJ_TRUE)
	{
		...
		if (EVENT_GENERATOR_touch_released() == (int32_t)LLUI_INPUT_OK)
		{
			// the event has been managed: we can store the new touch state
