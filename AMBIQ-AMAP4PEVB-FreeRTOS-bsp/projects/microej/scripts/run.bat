...
"%IAREW_INSTALLATION_DIR%\common\bin\cspybat" --download_only "%IAREW_INSTALLATION_DIR%\arm\bin\armproc.dll" "%IAREW_INSTALLATION_DIR%\arm\bin\armjlink2.dll" "%IAREW_PROJECT_DIR%\%IAREW_PROJECT_CONFIGURATION%\Exe\%IAREW_PROJECT_NAME%.out"  --plugin="%IAREW_INSTALLATION_DIR%\arm\bin\armbat.dll" --device_macro="%IAREW_INSTALLATION_DIR%\%IAREW_PROJECT_DEVICE_MACRO%" --backend "--endian=little" "--cpu=Cortex-M4" "--fpu=VFPv4_SP" "-p"  "%IAREW_INSTALLATION_DIR%/%IAREW_PROJECT_DEVICE_DESCRIPTION_FILE%" "--semihosting" "--device=%IAREW_PROJECT_DEVICE%" "--drv_communication=USB0" "--drv_interface_speed=auto" "--jlink_initial_speed=32" "--jlink_reset_strategy=0,0" "--drv_interface=SWD" "--drv_catch_exceptions=0x000" "--drv_swo_clock_setup=180000000,0,2000000" 